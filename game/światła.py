...
while(ll):
 ...
 while (LIFE and GAME):
   ...
   if ruchy < limit-int(limit/4): #7*(36+poziom*36+(poziom-4)*5)/10:
    label = myfont.render("Zamiany:", 5, biały)
    ekran.blit(label, (620, 250))
    label = myfont.render("  "+str(int(ruchy))+"/"
                              +str(int(limit-ruchy)),5, biały)
    ekran.blit(label, (650, 275))
   else:
    label = myfont.render("Zamiany:", 5, czerwony)
    ekran.blit(label, (620, 250))
    label = myfont.render("  "+str(int(ruchy))+"/"
                              +str(int(limit-ruchy)),5, czerwony)
